...
def check_patterns(seq):
    patterns = [
        re.compile(r'ATATCCAGAACCCTGACCC', re.IGNORECASE),
        re.compile(r'CAGTCGTGCATAGCAGC', re.IGNORECASE),
        re.compile(r'AACAC', re.IGNORECASE),
        re.compile(r'CAGGTCCTC', re.IGNORECASE)
    ]
    return all(pattern.search(seq) for pattern in patterns)


def extract_umis_and_remove_pattern(seq):
    pattern = r'ATATCCAGAACCCTGACCC([ACGT]{8})CAGTCGTGCATAGCAGC([ACGT]{8})ACAC[GC]TT[GT]TTCAGGTCCTC'
...
#     # Create a new SeqRecord with the remaining sequence
#     remaining_seq = seq[:match.start()] + seq[match.end():]
#     new_record = SeqRecord(
#         Seq(remaining_seq),
#         id=record.id,
#         name=record.name,
#         description=record.description + f" {umi}",
#         letter_annotations={"phred_quality": record.letter_annotations["phred_quality"][:match.start()] + record.letter_annotations["phred_quality"][match.end():]}
#     )
    
#     return umi, new_record
...
def main():
    ...
    barcode_file = "Beita.barcodes.txt"
